...
```
prepare.py (FIXED)           →  emovi_analytical.parquet
                                          ↓
...
Designed for **ESRU-EMOVI 2023** from Mexico (Centro de Estudios Espinosa Yglesias), but the pipeline architecture generalizes to any household survey with circumstance and income variables.
...
The pipeline produces `data/processed/emovi_analytical.parquet` with all 33 circumstance variables pre-coded.
...
├── imputation/                  # MICE via miceforest (M imputed datasets)
│   └── mice_imputer.py          # Create, save, load imputed datasets
├── tests/                        # Unit + integration + e2e tests
├── data/
│   ├── raw/                      # ESRU-EMOVI .dta files (gitignored)
│   ├── processed/                # .parquet analytical dataset (gitignored)
│   └── codebook.json             # Variable metadata (generated by prepare.py, gitignored)
├── results/                      # Generated: experiment logs + figures + tables (gitignored)
├── docs/
